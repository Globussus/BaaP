...
void Calculate() {
  ...
  if (std::cin.fail()) {
    std::cout << "Ошибка ввода! Повторите попытку." << std::endl;
		CleanInput();
  } else if (static_cast<int>(n) != n || n < 0 || static_cast<int>(k) != k || k < 0) {
    ...
    int* array_of_even = new int[amount_of_even];
    
    CreateArrayOfEven(matrix, array_of_even, n, k, amount_of_even);

    long long multiply = MultiplicationOfMatrix(matrix, n, k);
    std::cout << "Произведение элементов матрицы: " << multiply << std::endl;

    for (int i = 0; i < n; i++) {
      delete[] matrix[i];
    }
    delete[] matrix;
    delete[] array_of_even;
  }
}
